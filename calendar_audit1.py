...
def get_calendar_data(num_months):
    today = datetime.today()
    three_months_ago = today - relativedelta(months=num_months)
    tmin = three_months_ago.isoformat('T') + "Z"
    tmax = today.isoformat('T') + "Z"
    return (tmin,tmax)

def display_instance_date_info(event_id, calendar_id, start, end, recurrent, all_day_event, recurr_event_id, items):    
    for item in items:
        ...
        yield AuditEvent(recurr_event_id, calendar_id, start, end, recurrent, all_day_event)
...
class AuditEvent:
    def __init__(self, id, calendar_id, start, end, recurrent, all_day_event=False):
        self._event_id = id
...
def main():
    ...
    try:
        ...
        num_months = THREE
        tmin, tmax = get_calendar_data(num_months)
        ...
        for event in events:
            ...
            audit_event = AuditEvent(event['id'], calendar_id, start, end, recurrent, all_day_event)
            audit_event.set_attendees(attendees)
            if instances is not None:
                ...
                print("*************")
                total_duration += num_occurrences*audit_event.get_meeting_duration()
            else:
                total_duration += audit_event.get_meeting_duration()
            #print(audit_event._dict_)
            #print(audit_event.get_calendar_id())
